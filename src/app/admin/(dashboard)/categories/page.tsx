  ...
  return (
    <div>
      <div className="mb-8 flex flex-wrap items-center justify-between gap-4">
        <div>
          <h1 className="font-serif text-3xl font-light text-ink">Categories</h1>
              ...
              {categories.map((c) => (
                <tr key={c.id} className="border-b border-ink/5 last:border-0 hover:bg-paper-warm/50">
                  <td className="px-4 py-3">
                    <Link href={`/admin/categories/${c.id}/edit`} className="font-medium text-ink hover:text-gold-dark">
                      {c.name}
                    </Link>
                  </td>
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center justify-end gap-4">
                      <Link href={`/admin/categories/${c.id}/edit`} className="text-xs text-ink/65 hover:text-ink">
                        Edit
                      </Link>
                      <DeleteEntityButton id={c.id} name={c.name} action={deleteCategory} />
                    </div>
                  </td>
                </tr>
              ))}
