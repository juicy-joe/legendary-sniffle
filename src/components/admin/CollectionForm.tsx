...
import { createCollection, updateCollection } from "@/app/admin/(dashboard)/collections/actions";

type CollectionData = { id: string; name: string; slug: string; description: string | null };
...
export default function CollectionForm({
  ...
  const [state, formAction, pending] = useActionState(
    mode === "create" ? createCollection : updateCollection,
    {}
  );
  ...
  return (
    <form action={formAction} className="max-w-xl space-y-6">
      {mode === "edit" && <input type="hidden" name="id" value={collection!.id} />}
